...
double stats::computeCorCoeff(std::vector<double> inX,
...
double stats::computeCoVar(std::vector<double> inX, std::vector<double> inY,
                           double mX, double mY) {
    // make a vector to put in differences
    vector<double> diff(inX.size(), 0.0);
    // fill with differences
    std::transform(inX.begin(), inX.end(), inY.begin(), diff.begin(),
...
double stats::computeCorCoeffPop(std::vector<double> inX,
                                 std::vector<double> inY, double mX,
                                 double mY) {
    // convert to percent
    double mXp = mX * 100;
    double mYp = mY * 100;
    ...
    // cout << "stdDX: " << stdDevX << " stdDy: " << stdDevY << " cov " << covar
    // << endl;
    return corr;
}

double stats::computeCorCoeffSample(std::vector<double> inX,
                                    std::vector<double> inY) {
...
double stats::computeChiSqGoodFit(vector<double> obs, vector<double> exp) {
    ...
    std::transform(obs.begin(), obs.end(), exp.begin(), obs.begin(),
